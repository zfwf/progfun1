...
class ListsSuite {
  ...
  @Test def `sum of a few repeated numbers (10pts)` : Unit = {
    assert(sum(List(7, 7, 7)) == 21)
  }
  ...
  @Test def `max of a few negative numbers and zero` : Unit = {
    assert(max(List(-3, -7, 0, -2)) == 0)
  }

  @Test def `max of a few repeated numbers` : Unit = {
